...
def test_classifier_single_clear_match() -> None:
    ...
    detectors: list[BaseDetector] = [
        MockDetector(AlgorithmPattern.TWO_POINTER, 0.95),
        MockDetector(AlgorithmPattern.SLIDING_WINDOW, 0.0),
    ]
    classifier = PatternClassifier(detectors=detectors)
    results = classifier.classify("dummy code")

    assert len(results) == 1
    assert results[0].pattern == AlgorithmPattern.TWO_POINTER
...
def test_classifier_multiple_matches_ranked() -> None:
    ...
    detectors: list[BaseDetector] = [
        ...
        MockDetector(AlgorithmPattern.BFS_DFS, 0.9),
    ]
    classifier = PatternClassifier(detectors=detectors)
    results = classifier.classify("dummy code")

    assert len(results) == 2
    assert results[0].pattern == AlgorithmPattern.BFS_DFS
...
def test_classifier_no_match() -> None:
    ...
    detectors: list[BaseDetector] = [
        MockDetector(AlgorithmPattern.TWO_POINTER, 0.0),
        MockDetector(AlgorithmPattern.SLIDING_WINDOW, 0.0),
    ]
    classifier = PatternClassifier(detectors=detectors)
    results = classifier.classify("dummy code")
...
def test_classifier_overlap_handling() -> None:
    ...
    detectors2: list[BaseDetector] = [
        MockDetector(AlgorithmPattern.TWO_POINTER, 0.9),
        MockDetector(AlgorithmPattern.SLIDING_WINDOW, 0.7),
    ]
    classifier2 = PatternClassifier(detectors=detectors2)
    results2 = classifier2.classify("dummy code")

    assert len(results2) == 2
    assert results2[0].pattern == AlgorithmPattern.TWO_POINTER
